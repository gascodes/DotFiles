#!/usr/bin/env bash
# clean_update.sh — Limpieza y actualización interactiva
...
# Comprueba que se ejecute en un sistema tipo Debian/Ubuntu (apt) o Fedora (dnf/yum)
detect_pkg_mgr() {
...
if [ -z "$PKG_MGR" ]; then
  printf "%s No se ha detectado un gestor de paquetes compatible (apt/dnf/yum).\n" "$ICON_WARN"
  exit 1
fi
